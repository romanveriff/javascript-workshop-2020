!((d, w) => {
  d.addEventListener("DOMContentLoaded", () => {
      ...
      switch (action) {
        case "next": {
          return {
            ...model,
            cursor:
              model.cursor === model.slides.length - 1
        ...
        case "loop": {
          return {
            ...model,
            cursor:
              model.cursor === model.slides.length - 1 ? 0 : model.cursor + 1,
          };
        }
        case "loopback": {
          return {
            ...model,
            cursor:
              model.cursor === 0 ? model.slides.length - 1 : model.cursor - 1,
          };
        }
        case "prev":
          return {
    ...
    _app({
      title: d.title,
      cursor: w.location.hash ? Number(w.location.hash.replace("#", "")) - 1 : 0,
